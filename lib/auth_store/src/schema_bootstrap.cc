...
constexpr int kLatestMigrationVersion = 1;
...
std::vector<std::filesystem::path> candidate_migration_dirs(
    const std::filesystem::path& executable_path) {
    const auto executable_dir = executable_directory(executable_path);
    return {
...
int metadata_version(SqliteConnection& connection, std::string_view key) {
    ...
    constexpr auto kSql = "SELECT value FROM schema_metadata WHERE key = ?1 LIMIT 1;";
    if (sqlite3_prepare_v2(connection.handle(), kSql, -1, &statement, nullptr) != SQLITE_OK) {
...
void set_metadata_version(SqliteConnection& connection, std::string_view key, int version) {
...
std::map<int, std::filesystem::path> load_migration_files(
    const std::filesystem::path& executable_path) {
...
void apply_migrations(SqliteConnection& connection, const std::filesystem::path& executable_path) {
...
BootstrapResult bootstrap_at_path(const std::filesystem::path& database_path,
    ...
    if (!file_existed) {
        set_metadata_version(connection, "migration_version", kLatestMigrationVersion);
    } else {
        const bool has_plain_api_key_column = connection.column_exists("api_keys", "key_plain");
        if (has_plain_api_key_column) {
            set_metadata_version(connection, "migration_version", 0);
        } else if (metadata_version(connection, "migration_version") == 0) {
            set_metadata_version(connection, "migration_version", kLatestMigrationVersion);
        }
