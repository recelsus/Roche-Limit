...
public:
    ...
    virtual std::vector<IpRuleRecord> list_ip_rules(IpRuleEffect effect) const = 0;

    virtual std::optional<IpServiceLevelRecord> find_ip_service_level(
        std::int64_t ip_rule_id,
        std::string_view service_name) const = 0;

    virtual std::optional<ApiKeyRecord> find_api_key(
