...
std::filesystem::path test_database_path(std::string_view name) {
    auto dir = std::filesystem::temp_directory_path() / "roche-limit-auth-store-tests";
...
std::string scalar_text(const std::filesystem::path& database_path, const char* sql) {
    SqliteConnection connection(database_path);
    sqlite3_stmt* statement = nullptr;
    if (sqlite3_prepare_v2(connection.handle(), sql, -1, &statement, nullptr) != SQLITE_OK) {
...
void test_bootstrap_creates_current_schema() {
    ...
    expect(scalar_text(database_path,
                       "SELECT value FROM schema_metadata WHERE key = 'migration_version';") == "1",
...
INSERT INTO schema_metadata (key, value) VALUES ('migration_version', '1');
...
void test_api_key_repository_stores_hash_and_prefix_only() {
    ...
    expect(records.front().id == id, "listed api key id should match inserted id");
    expect(records.front().key_hash == key_hash, "api key hash should be persisted");
    expect(records.front().key_prefix.has_value() && *records.front().key_prefix == "test-api",
           "api key prefix should be persisted");

    const auto found = repository.find_api_key(key_hash, "primary");
    expect(found.has_value(), "api key should be found by hash and service");
...
void test_ip_remove_deletes_service_levels() {
    ...
    expect(scalar_int(database_path, "SELECT COUNT(*) FROM ip_service_levels;") == 0,
