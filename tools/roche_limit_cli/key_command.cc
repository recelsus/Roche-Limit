...
std::string api_key_prefix(std::string_view plain_api_key) {
  ...
  return std::string(
      plain_api_key.substr(0, std::min(kPrefixLength, plain_api_key.size())));
}

} // namespace

void handle_key_command(const RuleRepository &repository,
  ...
  if (action == "list") {
    ...
    print_table({"id", "prefix", "service", "level", "status", "note"}, rows);
    return;
  }
